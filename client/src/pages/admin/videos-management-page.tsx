...
const VideosManagementPage: React.FC = () => {
  ...
  const [videoToRename, setVideoToRename] = useState<VideoType | null>(null);
  ...
  const uploadMutation = useMutation({
    mutationFn: (file: File) => videoService.uploadVideo(file),
    onSuccess: () => {
  ...
  const renameMutation = useMutation({
    mutationFn: ({ id, newName }: { id: number, newName: string }) => videoService.renameVideo(id, newName),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/videos'] });
      toast({
        title: "Succès",
        description: "La vidéo a été renommée avec succès",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleRenameClick = (video: VideoType) => {
  ...
  const handleRenameConfirm = () => {
    if (videoToRename && newVideoName.trim()) {
      renameMutation.mutate({ id: videoToRename.id, newName: newVideoName.trim() });
  ...
  const handlePreviewClick = (video: VideoType) => {
  ...
  return (
    <AdminLayout
      title={t('admin.videos.title') || "Gestion des Vidéos"}
      currentPage="videos" // @ts-ignore - La valeur 'videos' est maintenant valide dans l'interface AdminLayoutProps
    >
      <div className="mb-6">
        ...
        <p className="mt-2 text-gray-600">
          Téléchargez, organisez et gérez les vidéos de votre site web.
        </p>
        <div className="mt-4">
          <BackendSelector />
        </div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        ...
        <Button className="bg-primary hover:bg-primary/90 w-full sm:w-auto order-1 sm:order-2">
          <label className="flex items-center cursor-pointer justify-center w-full">
            ...
            {uploadMutation.isPending ? 'Téléchargement...' : 'Ajouter une vidéo'}
            <input 
              type="file" 
              accept="video/*" 
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg border border-dashed border-gray-300">
          ...
          <h3 className="mt-2 text-sm font-semibold text-gray-900">Aucune vidéo</h3>
          <p className="mt-1 text-sm text-gray-500">
            Commencez par télécharger votre première vidéo.
          </p>
          <div className="mt-6">
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5">
          {videos.map((video) => (
            <div key={video.id} className="w-full bg-white rounded-lg shadow-sm transition-shadow hover:shadow-md">
              <div 
                className="aspect-video bg-gray-900 overflow-hidden rounded-t-lg flex items-center justify-center relative cursor-pointer"
              ...
              >
                {video.thumbnailUrl ? (
                  ...
                  <div className="flex flex-col items-center justify-center text-white/70">
                    ...
                    <div className="text-xs text-center">Cliquez pour prévisualiser</div>
                  </div>
                )}
              </div>
              <div className="p-3 sm:p-4">
                <h3 className="font-medium text-sm sm:text-base truncate" title={video.originalName}>
                  {video.originalName}
                </h3>
        ...
        <DialogContent className="max-w-[95vw] sm:max-w-md">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0">
            ...
            <Button 
              ...
              disabled={renameMutation.isPending || !newVideoName.trim()}
            >
      ...
      <Dialog open={previewDialogOpen} onOpenChange={setPreviewDialogOpen}>
        <DialogContent className="max-w-[95vw] max-h-[95vh] sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle className="truncate">{previewVideo?.originalName}</DialogTitle>
          </DialogHeader>
          <div className="py-2">
            {previewVideo && (
